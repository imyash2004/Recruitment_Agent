...
class ResumeAnalysisAgent:
    def __init__(self, api_key, cutoff_score=40):
    ...
    def extract_text_from_file(self, file):
        """Extract text from a file (PDF/TEXT)"""
    ...
    def create_rag_vector_store(self, text):
        ...
        embeddings = OpenAIEmbeddings(model="text-embedding-ada-002", openai_api_key=self.api_key)
        vectorstore = FAISS.from_texts(chunks, embeddings)
    ...
    def analyze_skill(self, qa_chain, skill):
        ...
        # With error handling:
        try:
            qa_response = qa_chain.invoke({"query": query}, config={'timeout': 60})
            response = qa_response.get("result") or qa_response.get("answer") or str(qa_response)
        except Exception as e:
        ...
        if match:
            ...
            return skill, 0, "Score not found in response"  # Default return if score not found
    ...
    def analyze_resume_weaknesses(self):
        """analyze specific weaknesses in the resume based on missing skilld"""
        ...
        if missing_skills:
            ...
            prompt = f"""
                Analyze why the resume is weak in demonstrating proficiency in the following skills: {', '.join(missing_skills)}.
            ...
            try:
                ...
                for skill_analysis in weakness_data.get("skill_analysis", []):
                    skill = skill_analysis.get("skill")
                    weakness_detail = {
                        "skill": skill,
                        "score": self.analysis_result.get("skill_scores", {}).get(skill, 0),
                        "detail": skill_analysis.get("weakness", "no specific details provided."),
                        "suggestions": skill_analysis.get("improvement_suggestions", []),
                        "example": skill_analysis.get("example_addition", "")
                    }
                ...
                print(f"JSONDecodeError: {weakness_content}")
                for skill in missing_skills:
                    weaknesses.append(
                        {
                            "skill": skill,
                            "score": self.analysis_result.get("skill_scores", {}).get(skill, 0),
                            "detail": "No specific details provided due to parsing error."
    ...
    def extract_skills_from_jd(self, jd_text):
        """Extract text from a direct job decscription"""

        try:
            llm = ChatOpenAI(model="gpt-3.5-turbo", api_key=self.api_key)
            prompt = f"""
            Extract a comprenhsive list of technical skills , technologies, and 
            competeencies required from this job description.
            Format the ouput as a Python list of strings. Only include the list ,
            nothing else .
            Job Description:
            {jd_text}"""
            ...
            match = re.search(r'\[(.*?)\]', skill_text, re.DOTALL)

            if match:
                skill_text = match.group(0)

            try:
                skills_list = eval(skill_text)
    ...
    def semantic_skill_analysis(self, resume_text, skills):
        ...
        reasoning = ("Candidate evaluated based on explicit resume content using semantic similarity and clear nueric "
                     "scoring.")
    ...
    def analyze_resume(self, resume_file, role_requirements=None, custom_id=None):
        ...
        if custom_id:
            self.jd_text = self.extract_text_from_file(custom_id)
            ...
            self.analysis_result = self.semantic_skill_analysis(self.resume_text,
                                                                role_requirements)
    ...
    def ask_question(self, question):
        ...
        if not self.rag_vectorstore or not self.resume_text:
            return "please analyse a resume first."
    ...
    def get_improved_resume(self, target_role="", highlight_skills=""):
        ...
        try:
            ...
            llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.7, api_key=self.api_key)
