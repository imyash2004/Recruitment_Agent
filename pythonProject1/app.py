...
def analyze_resume(agent, resume_file, role, custom_jd, upload_jd):
...
def main():
    ...
    with tabs[0]:
        role, custom_jd, upload_jd = ui.role_selection_section(ROLE_REQUIREMENTS)
        ...
        with col2:
            if st.button("🔍 Analyze Resume", type="primary"):
                if agent and uploaded_resume:
                    ...
                    analyze_resume(agent, uploaded_resume, role, custom_jd, upload_jd)
    ...
    with tabs[4]:
        if st.session_state.resume_analyzed and st.session_state.resume_agent:
            ui.improved_resume_section(
                has_resume=True,
                get_improved_resume_func=lambda role, skills: get_improved_resume(st.session_state.resume_agent, role,
                                                                                  skills)
            )
        else:
            st.warning("Please upload and analyze a resume first in the 'Resume Analysis' tab.")
